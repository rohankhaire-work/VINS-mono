...
namespace camodocal
{
  ...
  {
    mCornersFound
      = findChessboardCorners(mImage, mBoardSize, mCorners,
  ...
  bool
  Chessboard::findChessboardCorners(const cv::Mat &image, const cv::Size &patternSize,
  ...
  //===========================================================================
  void Chessboard::generateQuads(std::vector<ChessboardQuadPtr> &quads, cv::Mat &image,
                                ...
                                ChessboardQuadPtr &quad2, int corner2) const
  {
    ...
    // sides 1
    float x1
      = (quad1->corners[corner1]->pt.x + quad1->corners[(corner1 + 1) % 4]->pt.x) / 2;
    float y1
      = (quad1->corners[corner1]->pt.y + quad1->corners[(corner1 + 1) % 4]->pt.y) / 2;
    ...
    float x3
      = (quad1->corners[corner1]->pt.x + quad1->corners[(corner1 + 3) % 4]->pt.x) / 2;
    float y3
      = (quad1->corners[corner1]->pt.y + quad1->corners[(corner1 + 3) % 4]->pt.y) / 2;
    ...
    // quad sides 1
    float u1
      = (quad2->corners[corner2]->pt.x + quad2->corners[(corner2 + 1) % 4]->pt.x) / 2;
    float v1
      = (quad2->corners[corner2]->pt.y + quad2->corners[(corner2 + 1) % 4]->pt.y) / 2;
    ...
    // compute midpoints of "parallel" quad sides 2
    float u3
      = (quad2->corners[corner2]->pt.x + quad2->corners[(corner2 + 3) % 4]->pt.x) / 2;
    float v3
      = (quad2->corners[corner2]->pt.y + quad2->corners[(corner2 + 3) % 4]->pt.y) / 2;
