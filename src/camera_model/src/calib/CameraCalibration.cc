...
namespace camodocal
{
  ...
  {
    m_camera
      = CameraFactory::instance()->generateCamera(modelType, cameraName, imageSize);
  }
                                          ...
                                          std::vector<cv::Mat> &tvecs) const
  {
    ...
    for(size_t i = 0; i < m_scenePoints.size(); ++i)
    {
      camera->estimateExtrinsics(m_scenePoints.at(i), m_imagePoints.at(i), rvecs.at(i),
                                 tvecs.at(i));
    }
                                   ...
                                   std::vector<cv::Mat> &tvecs) const
  {
    ...
    for(size_t i = 0; i < rvecs.size(); ++i)
    {
      ...
      transformVec.at(i).translation() << tvecs[i].at<double>(0), tvecs[i].at<double>(1),
        tvecs[i].at<double>(2);
    }
