    ...
    if(corres.size() > 20)
    {
         ...
         && m_estimator.solveRelativeRT(corres, relative_R, relative_T))
      {
        ...
        std::cout
          ...
          << " and newest frame to triangulate the whole structure, corres.size():"
          << corres.size() << std::endl;
        return true;
      }
    }
  }
...
void Estimator::double2vector()
{
  ...
  for(int i = 0; i <= WINDOW_SIZE; i++)
  {
    ...
    Ps[i]
      = rot_diff
          * Vector3d(para_Pose[i][0] - para_Pose[0][0], para_Pose[i][1] - para_Pose[0][1],
  ...
  if(LOOP_CLOSURE && relocalize && retrive_data_vector[0].relative_pose
     && !retrive_data_vector[0].relocalized)
  {
    ...
    Vector3d vio_loop_t;
    vio_loop_r = rot_diff
                 * Quaterniond(retrive_data_vector[0].loop_pose[6],
                               retrive_data_vector[0].loop_pose[3],
                               retrive_data_vector[0].loop_pose[4],
                               retrive_data_vector[0].loop_pose[5])
                     .normalized()
                     .toRotationMatrix();
    vio_loop_t = rot_diff
                   * Vector3d(retrive_data_vector[0].loop_pose[0] - para_Pose[0][0],
                              retrive_data_vector[0].loop_pose[1] - para_Pose[0][1],
                              retrive_data_vector[0].loop_pose[2] - para_Pose[0][2])
                 + origin_P0;
    Quaterniond vio_loop_q(vio_loop_r);
    double relocalize_yaw;
    relocalize_yaw
      = Utility::R2ypr(retrive_data_vector[0].R_old).x() - Utility::R2ypr(vio_loop_r).x();
    ...
    for(int k = 0; k < (int)retrive_data_vector.size(); k++)
    {
      for(int i = 0; i < WINDOW_SIZE; i++)
      {
        if(retrive_data_vector[k].header == Headers[i].stamp.toSec())
        {
          ...
          ceres::LocalParameterization *local_parameterization
            = new PoseLocalParameterization();
          problem.AddParameterBlock(retrive_data_vector[k].loop_pose, SIZE_POSE,
          ...
          for(auto &it_per_id : f_manager.feature)
          {
            ...
            if(start <= i)
            {
              while(retrive_data_vector[k].features_ids[retrive_feature_index]
                    < it_per_id.feature_id)
              {
                retrive_feature_index++;
              }

              if(retrive_data_vector[k].features_ids[retrive_feature_index]
                 == it_per_id.feature_id)
              {
                ...
                problem.AddResidualBlock(f, loss_function, para_Pose[start],
                                         retrive_data_vector[k].loop_pose,
                                         para_Ex_Pose[0], para_Feature[feature_index]);
    ...
    for(int k = 0; k < (int)retrive_data_vector.size(); k++)
    {
      for(int i = 0; i < WINDOW_SIZE; i++)
      {
        if(retrive_data_vector[k].header == Headers[i].stamp.toSec())
        {
          ...
          Quaterniond Qs_loop;
          Qs_loop = Quaterniond(retrive_data_vector[k].loop_pose[6],
                                retrive_data_vector[k].loop_pose[3],
                                retrive_data_vector[k].loop_pose[4],
                                retrive_data_vector[k].loop_pose[5])
                      .normalized()
                      .toRotationMatrix();
          Matrix3d Rs_loop = Qs_loop.toRotationMatrix();
          Vector3d Ps_loop = Vector3d(retrive_data_vector[k].loop_pose[0],
          ...
          retrive_data_vector[k].relative_yaw = Utility::normalizeAngle(
            Utility::R2ypr(Rs_i).x() - Utility::R2ypr(Rs_loop).x());
          if(abs(retrive_data_vector[k].relative_yaw) > 30.0
             || retrive_data_vector[k].relative_t.norm() > 20.0)
