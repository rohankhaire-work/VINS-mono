...
#include "rosmsg/Path.h"
#include "rosmsg/Odometry.h"
#include "rosmsg/PointStamped.h"
...
public:
  ...
  int size();
  void optimize4DoFLoopPoseGraph(int cur_index, Eigen::Vector3d &loop_correct_t,
...
{
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  RelativeTError(double t_x, double t_y, double t_z) : t_x(t_x), t_y(t_y), t_z(t_z) {}
...
{
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  TError(double t_x, double t_y, double t_z) : t_x(t_x), t_y(t_y), t_z(t_z) {}

  template <typename T> bool operator()(const T *tj, T *residuals) const
  {
...
struct RelativeRTError
{
  ...
  Create(const double t_x, const double t_y, const double t_z, const double q_w,
...
struct FourDOFError
{
  ...
  FourDOFError(double t_x, double t_y, double t_z, double relative_yaw, double pitch_i,
               double roll_i)
      : t_x(t_x), t_y(t_y), t_z(t_z), relative_yaw(relative_yaw), pitch_i(pitch_i),
        roll_i(roll_i)
  {}
  ...
  static ceres::CostFunction *
  Create(const double t_x, const double t_y, const double t_z, const double relative_yaw,
...
struct FourDOFWeightError
{
                  ...
                  T *residuals) const
  {
    ...
    residuals[3]
      = NormalizeAngle((yaw_j[0] - yaw_i[0] - T(relative_yaw))) * T(weight) / T(10.0);
