...
namespace DLoopDetector
{
  ...
  public:
    ...
    struct Parameters
    {
      ...
      Parameters(int height, int width, float frequency = 1, bool nss = true,
                 float _alpha = 0.3, int _k = 1, GeometricalCheck geom = GEOM_DI,
                 int dilevels = 2);
  ...
  public:
     ...
     * @param db database to copy
     */
    template <class T> void setDatabase(const T &db);
    ...
    bool detectLoop(const std::vector<cv::KeyPoint> &keys,
                    ...
                    std::vector<cv::Point2f> &cur_pts, std::vector<cv::Point2f> &old_pts);
  ...
  protected:
    ...
    void reduceInputToOutput(vector<cv::Point2f> v_in, vector<cv::Point2f> &v_out,
                             vector<uchar> status);
     ...
     */
    bool
    isGeometricallyConsistent_DI(EntryId old_entry, const std::vector<cv::KeyPoint> &keys,
     ...
     */
    void
    getMatches_neighratio(const std::vector<TDescriptor> &A,
  ...
  template <class TDescriptor, class F>
  TemplatedLoopDetector<TDescriptor, F>::Parameters::Parameters(int height, int width,
                                                                float frequency, bool nss,
                                                                float _alpha, int _k,
  ...
  TemplatedLoopDetector<TDescriptor, F>::TemplatedLoopDetector(
    const TemplatedVocabulary<TDescriptor, F> &voc, const Parameters &params)
      : m_params(params)
  {
    m_database = new TemplatedDatabase<TDescriptor, F>(voc, params.geom_check == GEOM_DI,
                                                       params.di_levels);
  ...
  {
    delete m_database;
    m_database = new TemplatedDatabase<TDescriptor, F>(
      voc, m_params.geom_check == GEOM_DI, m_params.di_levels);
  }
  ...
  {
    m_database = new TemplatedDatabase<TDescriptor, F>(
      db.getVocabulary(), params.geom_check == GEOM_DI, params.di_levels);
  }
    ...
    else
    {
      ...
      if(!qret.empty())
      {
        ...
        if(!m_params.use_nss || ns_factor >= m_params.min_nss_factor)
        {
          ...
          // printf("ns_factor %lf >= m_params.min_nss_factor
              ...
              if(getConsistentEntries() > m_params.k)
              {
                ...
                if(m_params.geom_check == GEOM_DI)
                {
                  ...
                  detection = isGeometricallyConsistent_DI(
                    island.best_entry, keys, descriptors, featvec, cur_pts, old_pts);
                }
  ...
  template <class TDescriptor, class F>
  void
  TemplatedLoopDetector<TDescriptor, F>::computeIslands(QueryResults &q,
  ...
  template <class TDescriptor, class F>
  double
  TemplatedLoopDetector<TDescriptor, F>::calculateIslandScore(const QueryResults &q,
  ...
  template <class TDescriptor, class F>
  bool TemplatedLoopDetector<TDescriptor, F>::checkFoundamental(
    vector<cv::Point2f> cur_input_pts, vector<cv::Point2f> old_input_pts,
    vector<cv::Point2f> &cur_output_pts, vector<cv::Point2f> &old_output_pts)
  {
    if(cur_input_pts.size() >= 8)
    {
      ...
      vector<cv::Point2f> un_cur_pts(cur_input_pts.size()),
        un_old_pts(cur_input_pts.size());
      for(unsigned int i = 0; i < cur_input_pts.size(); i++)
      {
        Eigen::Vector3d tmp_p;
        m_camera->liftProjective(Eigen::Vector2d(old_input_pts[i].x, old_input_pts[i].y),
                                 tmp_p);
        tmp_p.x() = FOCAL_LENGTH * tmp_p.x() / tmp_p.z() + COL / 2.0;
        tmp_p.y() = FOCAL_LENGTH * tmp_p.y() / tmp_p.z() + ROW / 2.0;
        un_old_pts[i] = cv::Point2f(tmp_p.x(), tmp_p.y());

        m_camera->liftProjective(Eigen::Vector2d(cur_input_pts[i].x, cur_input_pts[i].y),
                                 tmp_p);
        tmp_p.x() = FOCAL_LENGTH * tmp_p.x() / tmp_p.z() + COL / 2.0;
        tmp_p.y() = FOCAL_LENGTH * tmp_p.y() / tmp_p.z() + ROW / 2.0;
  ...
  bool TemplatedLoopDetector<TDescriptor, F>::isGeometricallyConsistent_DI(
    ...
    const std::vector<TDescriptor> &descriptors, const FeatureVector &bowvec,
    std::vector<cv::Point2f> &cur_pts, std::vector<cv::Point2f> &old_pts)
  {
