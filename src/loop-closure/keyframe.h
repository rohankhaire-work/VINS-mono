...
public:
  ...
  virtual void
  operator()(const cv::Mat &im, const std::vector<cv::Point2f> window_pts,
...
public:
  ...
  bool
  searchInAera(cv::Point2f center_cur, float area_size,
               const BRIEF::bitset window_descriptor,
               const std::vector<BRIEF::bitset> &descriptors_old,
               const std::vector<cv::KeyPoint> &keypoints_old, cv::Point2f &best_match);
  ...
  void addConnection(int index, KeyFrame *connected_kf, Vector3d relative_t,
                     Quaterniond relative_q, double relative_yaw);

  void
  updateLoopConnection(Vector3d relative_t, Quaterniond relative_q, double relative_yaw);
