...
template <class TDescriptor> class FeatureExtractor
{
public:
...
public:
   ...
   */
  bool run(const std::string &name, const std::vector<cv::KeyPoint> &keys,
           const std::vector<TDescriptor> &descriptors, std::vector<cv::Point2f> &cur_pts,
           std::vector<cv::Point2f> &old_pts, int &old_index);
...
demoDetector<TVocabulary, TDetector, TDescriptor>::demoDetector(
  const std::string &vocfile, int width, int height)
    : m_vocfile(vocfile), m_width(width), m_height(height), params(height, width),
      voc(vocfile), detector(voc, params)
{
