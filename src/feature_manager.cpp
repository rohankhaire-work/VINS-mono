  ...
  int frame_count, const map<int, vector<pair<int, Vector3d>>> &image)
{
  ...
  for(auto &id_pts : image)
  {
    ...
    auto it
      = find_if(feature.begin(), feature.end(), [feature_id](const FeaturePerId &it) {
...
void FeatureManager::setDepth(const VectorXd &x)
{
  ...
  for(auto &it_per_id : feature)
  {
    ...
    // ROS_INFO("feature id %d , start_frame %d, depth %f ", it_per_id->feature_id,
...
{
  for(auto it = feature.begin(), it_next = feature.begin(); it != feature.end();
      it = it_next)
  {
    it_next++;
    if(it->solve_flag == 2)
...
double
FeatureManager::compensatedParallax2(const FeaturePerId &it_per_id, int frame_count)
{
